...
import { BOARD_INFO_KEY, DEFAULT_BOARD_INFO } from './consts.js'
...
export function createNewJobPost(step, newJobInfo) {
  boardInfo.update((info) =>
    info.map(({ headline, items }) =>
      headline === step ? { headline, items: [newJobInfo, ...items] } : { headline, items },
    ),
  )
  persistBoardInfo()
}
