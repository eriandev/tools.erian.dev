<script>
  ...
  const labelClass =
    'relative block w-10 cursor-pointer select-none overflow-hidden rounded bg-theme-gray-200 pb-8 outline-0 transition-[left,right,background-color] duration-200 ease-in-out [-webkit-backface-visibility:hidden] [backface-visibility:hidden]'
  const afterClass =
  ...
  const peerClass =
    'peer-checked:bg-theme-primary peer-checked:before:-left-full peer-checked:after:left-0 peer-checked:active:after:left-[10%]'
</script>

<div class="flex w-max flex-col gap-y-[6px] text-theme-gray-400{disabled ? ' opacity-70' : ''}">
  {#if label}
